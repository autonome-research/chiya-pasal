/**
 * v2 librarian phases. The new shape treats sources as first-class wiki
 * pages and topics as routing nodes. See src/librarian-v2.ts for the phase
 * composition and how these compose.
 *
 * Phases below are organized in execution order:
 *   - reapStaleV2 + loadBatchV2: queue management.
 ...
 *   - mergeMetadataV2 + commitLocalV2 close out the batch.
 */
...
import type {
  ...
  ExtractedRefs,
  LibrarianV2Ctx,
} from '../shared/librarian-v2-types.js';
...
// ---------------------------------------------------------------------------
// reapStaleV2 + loadBatchV2 — copies of v1's reapStale and loadBatch typed
// against LibrarianV2Ctx. v1's versions are bound to LibrarianCtx; the
// underlying logic is identical and equally applicable to either pipeline.
// Kept as separate exports rather than parameterizing v1 to avoid risk to
// the production librarian during the v2 cutover window.
// ---------------------------------------------------------------------------

const STALE_PROCESSING_MINUTES = 60;

export const reapStaleV2 = (store: ArticleStore): Phase<LibrarianV2Ctx> => ({
  name: 'reap-stale',
  async *run(ctx) {
...
export const loadBatchV2 = (store: ArticleStore): Phase<LibrarianV2Ctx> => ({
  name: 'load-batch',
  async *run(ctx) {
...
export const batchEnrich = (): Phase<LibrarianV2Ctx> => ({
  name: 'batch-enrich',
  async *run(ctx) {
...
export const batchExtractRefs = (): Phase<LibrarianV2Ctx> => ({
...
export const perArticleTree =
  (
    ...
    async *run(ctx) {
      ...
      // Map FanOutResult<ArticleResultV2>[] → ArticleResultV2[]; same shape
      // as v1's processBatch post-loop. Synthetic AbortError → soft deadline.
...
// ---------------------------------------------------------------------------
// mergeMetadataV2 — append per-article logEntries to log.md, no index
// deltas (v2 doesn't maintain index.md per-article — separate lint).
// ---------------------------------------------------------------------------

export const mergeMetadataV2 = (vault: VaultFs): Phase<LibrarianV2Ctx> => ({
  name: 'merge-metadata',
  async *run(ctx) {
...
// ---------------------------------------------------------------------------
// commitLocalV2 — single local commit per run, scoped to the v2 paths.
// ---------------------------------------------------------------------------

export const commitLocalV2 = (git: GitOps): Phase<LibrarianV2Ctx> => ({
  name: 'commit-local',
  async *run(ctx) {
    ...
    const message = `ingest-v2: ${results.length} articles (${tally.done} done, ${tally.skipped} skipped, ${tally.failed} failed)`;
    // v2 paths: source pages, topic pages, backlinks, log.
